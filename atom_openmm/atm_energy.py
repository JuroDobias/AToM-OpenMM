...
def reconstruct_atm_energies(
    ...
    *,
    ...
    reference_base = u0 if float(reference_direction) >= 0.0 else u1
    environment = float(reference_total_energy) - float(reference_bias_energy) - float(
        reference_base
    )
    return np.asarray(
        [
